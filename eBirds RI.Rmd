...
get_count_per_season <- function(df, year) {
  seasons <- unlist(c("spring", "breeding", "fall", "winter", "all"))
  counts <- unlist(map(seasons, function(x) nrow(get_bird_sightings_for_season_year(df, x, year))))
  years <- rep(year, times=length(seasons))
  data.frame(cbind(years, seasons, counts))
}
...
get_count_per_season_for_year_range(weekapaug_df, 2018, 2019)
