...
get_bird_sightings_for_season_year <- function(df, season, year) {
  ...
  # Args:
  #   df - the dataframe to filter
  #   season - one of ['spring', 'breeding', 'fall', 'winter']
  #   year - the year to filter for
  # Return:
  #   A data frame containing the bird sightings from the given df, filtered for the
  #   specified season and year
  
  if (!season %in% c("spring", "breeding", "fall", "winter"))
    stop("season arg must be one of ['spring', 'breeding', 'fall', 'winter']")
  
  if (year < 1970)
    stop("year must be between 1970-present")
  
  start_month <- switch(season, "spring" = 3,"breeding" = 6,"fall" = 8,"winter" = 12)
  end_month <- switch(season, "spring" = 5, "breeding" = 7, "fall" = 11, "winter" = 2)
  start_year <- year
...
get_bird_sightings_for_season_year(rhode_island_df, "spring", 2019)
get_bird_sightings_for_season_year(rhode_island_df, "breeding", 2019)
get_bird_sightings_for_season_year(rhode_island_df, "fall", 2019)
get_bird_sightings_for_season_year(rhode_island_df, "winter", 2019)
```
